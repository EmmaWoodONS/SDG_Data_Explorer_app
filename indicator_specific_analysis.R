...
server <- function(input, output, session) {
  ...
  }) # Takes the innput file -- Please use 3.a.1 from CSV folder as example
  
  
  # This will allow the choice of the first disaggregtaion
  output$Choice1 <- renderUI({
    req(input$file1)
    selectInput("Choice1", "Select first disaggregation", names(Indicator())) 
  ...
  output$Choice2 <- renderUI({
    req(input$file1)
    selectInput("Choice2", "Select choice for this disaggregation", Indicator()[,input$Choice1])
  })
